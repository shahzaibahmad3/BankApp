...
public class GetStatement extends JFrame implements ActionListener {
	...
	public void ShowP() {
		...
		ImageIcon i=new ImageIcon("images\\b_stmt.png");
		ImageIcon p2=new ImageIcon("images\\stmt.png");
		...
		super.add(acc);
			...
			super.add(pass);
			 ...
			 check=new JButton();
			 check.setBounds(180,230,200,45);
			 check.setIcon(p2);
			 super.add(check);
			...
			String accno=acc.getText(),
				...
				if(!accno.isEmpty() && !pas.isEmpty()){
					...
					if(rs.next()) {
						...
						r.exec("C:\\Windows\\notepad.exe images\\Statement\\"+bt+"\\"+accno+".txt");
					}
					else {
