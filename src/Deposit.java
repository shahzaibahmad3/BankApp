...
public class Deposit extends JFrame implements ActionListener {
	...
	public void ShowP() {
		...
		ImageIcon i=new ImageIcon("images\\b_depo.png");
			...
			String accno=acc.getText(),
				...
				if(!accno.isEmpty() && !pas.isEmpty()){
					...
					if(rs.next()) {
						...
						int bal=rs.getInt("balance")+Integer.parseInt(bala);
						st.executeUpdate("update "+bt+" set balance='"+bal+"' where aac_no='"+accno+"' AND password=sha1('"+pas+"') ");
						DateAndTime.stmt(bt, accno, "Amount deposited  "+bala+" on  " );
						
						JOptionPane.showMessageDialog(this, "Amount Depositeimages "+bala+
								"\nNew Balance:  "+bal);
						this.dispose();
						HomePage hp=new HomePage();
						hp.ShowP();
							...
							JOptionPane.showMessageDialog(this, "Account is Deativated");
						}
